#!/usr/bin/env python3
"""v3 帧头 + msgpack body（对等中继）：与 C++ `pack_wire` / `deliver/client/include/fwd/protocol.hpp` 对齐。"""
...
def pack_frame_msgpack(
    ...
    return pack_header(len(body), msg_type, seq) + body


def unpack_header(hb: bytes) -> dict:
    magic, ver, hlen, blen, typ, seq, r0 = struct.unpack("<IHHIIII", hb)
    return {
        "magic": magic,
        "version": ver,
        "header_len": hlen,

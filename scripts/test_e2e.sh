#!/usr/bin/env bash
# 端到端：临时端口启动 asio_forwarder（对等中继）→ Python relay_e2e_runner（LOGIN/TRANSFER）→ admin /api/health
set -euo pipefail
...
python3 -c "import msgpack" 2>/dev/null || {
  echo "installing msgpack for e2e: pip install -r tools/requirements-relay.txt" >&2
  pip install -q -r "$ROOT/tools/requirements-relay.txt"
}

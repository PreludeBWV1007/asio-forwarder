...
本文与 **`deliver/server/src/main.cpp`**、**`deliver/client/include/fwd/protocol.hpp`**、**`deliver/client/include/fwd/relay_constants.hpp`**、**`local/tools/forwarder_wire.py`**（Python 侧组帧参考）对齐。若与旧版文档或外部说明冲突，以本仓库代码为准。
...
布局与字段读写见 `proto::Header::pack_le` / `unpack_le`（`deliver/client/include/fwd/protocol.hpp`）。
...
### 客户端上行：`Header.msg_type`（`deliver/client/include/fwd/relay_constants.hpp`）
...
- 口令存储：进程内使用 **盐（hex）+ SHA-256（hex）** 摘要（见 `deliver/client/include/fwd/sha256.hpp`）；**重启后账号清空**。
...
见 `deliver/server/forwarder.json` 与 `deliver/client/include/fwd/config.hpp`：
...
其余：`/api/health`、`/api/stats`、`/api/events`（见根目录 `README.md` / [architecture.md](architecture.md)）。

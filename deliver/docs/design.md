...
| 组件 | 职责 |
|------|------|
| `asio_forwarder`（`deliver/server/src/main.cpp`） | 单 client acceptor + 多 `TcpSession`；协程读帧；账户与转发；admin acceptor |
| Python 工具（`local/tools/`） | 组帧/解析、自动化 e2e、套件测试、**交互式 `relay_cli.py`** |
...
见 **[local.md](local.md)**（`local/tests/`、`relay_cli`、样例压测表；**压测命令形态已随协议更新**）。

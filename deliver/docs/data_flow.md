...
主逻辑在 **`deliver/server/src/main.cpp`**：`TcpSession::read_loop`、`RelayServer::handle_client_frame`。
...
- **`msg_type == 4`（DATA）**：
  ...
  - 单播 / 广播到用户全连接 / 对该用户连接轮询：见 [protocol.md](protocol.md)。
...
- `deliver/client/include/fwd/protocol.hpp`：v2 头
- `deliver/client/include/fwd/relay_constants.hpp`：客户端 1–4、200 / 201 / 202
- `deliver/client/include/fwd/sha256.hpp`：口令摘要
- `deliver/client/include/fwd/config.hpp`、`deliver/server/forwarder.json`
- `local/tools/forwarder_wire.py`、`local/tools/relay_cli.py`

...
这样无论数据是一次性到达还是分多次到达（半包），只要按「先 24、再 body_len」循环读，就能**重新拼出完整帧**。这就是文档里说的**粘包/半包**处理思路。
...
[上游客户端] --TCP--> 本机 :19001
                    ...
                    upstream_readloop()  循环：
                      读 24B Header → 校验 → 读 body_len 的 Body
                          |
...
2. **读 Header（恰好 24 字节）**  
   使用 `read_exact_with_timeout`：在限定时间内必须读满 24 字节，否则取消读并失败。  
   读到的 24 字节放在 `std::array<std::uint8_t, 24>` 里。

3. **解析 Header**  
   `proto::Header::unpack_le` 把这 24 字节按**小端**解释成结构体字段（magic、version、body_len 等）。
...
7. **拼成「线上格式」的一整块**  
   - `h.pack_le()` 再生成 24 字节的 header 字节（保证与线上一致）；  
...
1. **一帧最少多少字节？**  
   只有 Header、Body 为空时，共 24 字节（`body_len == 0`）。
